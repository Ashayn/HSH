'use strict';
   ...
   */
   ...
   .controller('ExplorerCtrl', function ($q, $scope, $rootScope, itemMirror) {
...
$scope.createMultiplePhantoms = function (phantomRequest) {
    return itemMirror.createAssociation(phantomRequest);
  };
...
$scope.saveLinks = function($event){
  ...
  for(var i=0 ; i <= $scope.data.length; i++) {
        ...
        promises.push($scope.createMultiplePhantoms($scope.phantomRequest));

        $q.all(promises).then(function () {
          switchToAssocEditor();
            assocScopeUpdate();
            resetPhantomRequest();
            // alert("called promises");
          });
      ...
      $scope.editSectionFn = function ($event) {
      
        $scope.editSection = 'new-phantom';
